...
def signup():
    ...
    if request.method == "POST":
        ...
        print(f"  Email: {email}")
        print(f"  Phone: {phone}")
        print(f"  Hostel Type: {hostel_type}")
        print(f"  Password: {'*' * len(password) if password else 'None'}")
        print(f"  Confirm Password: {'*' * len(confirm_password) if confirm_password else 'None'}")
        
        # Check if all required fields are present
        if not all([name, email, phone, hostel_type, password, confirm_password]):
...
def clear_database():
    ...
    with db_lock:
        ...
        conn.commit()
        conn.close()
    flash("All users cleared from database!", "info")
...
if __name__ == "__main__":
    ...
    print("  - /clear-database (Clear all users for testing)")
    app.run(debug=True)